...
- **명시 승인 옵트인**: `gh pr merge`는 기본 차단이지만, 사용자가 승인하면 메인 세션 명령에 `PHOENIX_FLOW_ALLOW_MERGE=1`을 붙여 **PR 머지만** 통과시킬 수 있다(`PHOENIX_FLOW_ALLOW_MERGE=1 gh pr merge 1 --merge`). subagent는 예외 없음, 보호브랜치 push·이력 변조 차단은 그대로. 실수로는 안 붙는 토큰이라 "사람이 의도적으로 승인" 신호로 동작.
- **알려진 한계 — heredoc 메시지 false-positive**: 커밋/PR 메시지를 heredoc(`git commit -F - <<'EOF' … gh pr merge … EOF`)으로 줄 때, 본문에 `git push`·`gh pr merge` 같은 텍스트가 있으면 가드가 명령으로 보고 차단할 수 있다. 이는 보안상 의도된 **fail-closed**다 — heredoc 본문을 "데이터"로 보고 검사에서 빼면, 같은 본문이 `bash <<'EOF'`·`git()` 함수 재정의 등으로 **실제 실행**될 때 가드가 뚫린다(fail-open). 그래서 본문을 지우지 않는다.
  - **우회**: 메시지를 파일로 빼서 `git commit -F <파일>` / `gh pr create --body-file <파일>` 로 준다(heredoc 대신 파일 → 트리거 단어가 명령줄에 없음). Claude Code 라면 Write 도구로 메시지 파일을 만들고 `-F`/`--body-file` 로 참조하면 된다.
...
- **도구 격리**(v0.6.0): gemini·codex는 **저장소 밖 빈 임시 cwd**에서 실행한다(+ codex `--ignore-rules`·`--ignore-user-config`·`--ephemeral`, gemini `--skip-trust`). 모델이 주변 파일을 탐색하다 비밀을 끌어오는 표면을 줄인다. 임시 cwd 생성 실패 시 원 cwd로 폴백하며 그 경우 출력 메타에 `격리: 없음`으로 **정직하게** 표시한다. (요구: codex ≥ 0.133 / gemini ≥ 0.43 — 해당 플래그 지원 버전. 미지원이면 해당 도구가 `status=error`로 표시됨)
...
> **잔여 리스크(정직하게)**: 위 격리는 *표면 축소*지 하드 차단이 아니다. 읽기전용 플래그(codex `--sandbox read-only`, gemini `--approval-mode plan`)는 **쓰기만** 막고 읽기 범위는 안 막으므로, 모델이 절대경로로 저장소의 `.env` 등을 직접 읽는 것까진 못 막는다. 우리가 *넘기는 입력*은 안전하게 걸렀지만(위 정보유출 방어), 도구의 독립적 디스크 읽기를 강제로 막으려면 OS 샌드박스(macOS `sandbox-exec`로 repo subtree read-deny + canary 검증)가 필요하다 — 복잡도 대비 ROI를 보고 **후속 옵션**으로 둔다. "방어층 하나"라는 플러그인 철학과 같은 선.
