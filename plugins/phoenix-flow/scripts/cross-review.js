...
const INSTRUCTION =
  '다음 stdin 내용은 검토할 코드 변경(diff) 또는 파일 묶음이다. 읽기전용으로 검토만 하라.\n' +
  '버그·보안·품질 위주로 핵심 지적만 우선순위 높은 것부터. 제공된 내용만 보고, 저장소의 다른 파일을 ' +
  '읽거나 어떤 파일도 수정/실행하지 마라. 한국어로 간결하게.';
...
function main() {
  ...
  if (opts.tools.includes('gemini')) {
    tools.push(runTool('GEMINI', 'gemini',
      ['--approval-mode', 'plan', '-p', INSTRUCTION, '-o', 'text'],
      built.payload, opts.timeout, cwd));
  }
  if (opts.tools.includes('codex')) {
    tools.push(runTool('CODEX', 'codex',
      ['exec', '--sandbox', 'read-only', '--skip-git-repo-check', INSTRUCTION],
      built.payload, opts.timeout, cwd));
  }

  Promise.all(tools).then((results) => {
    process.stdout.write(render(built, results, opts));
  });
}
...
function runTool(label, bin, args, payload, timeoutMs, cwd) {
  return new Promise((resolve) => {
    ...
    child.on('error', (e) => {
      clearTimeout(timer);
      resolve({ label, bin, status: 'error', exitCode: null, ms: Date.now() - start, stdout: out.toString('utf8'), stderr: String(e.message || e) });
    });
    child.on('close', (code) => {
      clearTimeout(timer);
      ...
      resolve({ label, bin, status, exitCode: code, ms: Date.now() - start, stdout, stderr });
    });
...
function render(built, results, opts) {
  const lines = [];
  lines.push(`===== CROSS-REVIEW =====`);
  lines.push(`대상: ${built.targetDesc} | 입력 ${Buffer.byteLength(built.payload)}B | 도구: ${opts.tools.join(', ')}`);
